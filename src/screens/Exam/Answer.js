...
const Answer = (props) => {
      ...
      <View key={index} style={AnswerStyles.questionContainer}>
        ...
        <View style={AnswerStyles.questionHeightBox}>
          <View style={AnswerStyles.questionSubContainer}>
            <Text style={AnswerStyles.questionText}>
              {t(question.question)}
            </Text>
          </View>
          <Spacing />
          <Text style={AnswerStyles.indicateQuestion}>
            {currentQuestionIndex + 1} {t("Of_Label")} {questions.length}
          </Text>
        </View>
        <View style={AnswerStyles.questionOptionHeightBox}>
          {question.options.map((option, optionIndex) => (
            <View key={optionIndex} style={AnswerStyles.optionButton}>
              <VectorIcon
                icon="Feather"
              ...
              <Text
                style={[
                  option === selectedAnswer
                    ? isCorrect
                      ? AnswerStyles.correctOption
                      : AnswerStyles.incorrectOption
          ...
          <View>
            ...
            <TouchableOpacity style={AnswerStyles.flexRowAlCen}>
              <VectorIcon
                ...
                size={SF(20)}
              />
              <Text style={AnswerStyles.suggestionText}>
                {t("Answer_Label")}
              </Text>
            </TouchableOpacity>
            <Text style={AnswerStyles.answerSuggest}>
              {t(question?.answerBrief)}
            </Text>
          </View> */}
        </View>
        <Spacing />
  ...
  const renderCurrentQuestion = () => {
    ...
    return <View>{renderQuestion(currentQuestion, currentQuestionIndex)}</View>;
  };
  ...
  return (
    <Container>
      ...
      <View style={[AnswerStyles.headerViewBox, AnswerStyles.padH20]}>
        ...
        <TouchableOpacity
          style={[AnswerStyles.leftHead, AnswerStyles.rightHead]}
